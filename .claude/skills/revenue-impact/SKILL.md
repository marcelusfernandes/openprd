...
AskUserQuestion({
  questions: [
    {
      question: "Qual a base de usuários afetados (total ou por segmento)?",
      header: "Usuários Afetados"
    },
    {
      question: "Qual a receita média por usuário (ARPU) mensal? Ex: R$50, $120",
      header: "ARPU"
    },
    {
      question: "Quais métricas de conversão/retenção atuais você tem? (opcional)",
      header: "Métricas Atuais",
      options: [
        { label: "Tenho dados — vou informar", description: "Taxas de conversão, churn, retenção disponíveis" },
        { label: "Não tenho — usar estimativas", description: "Skill usará benchmarks conservadores tagados como [AI estimation]" }
      ]
    }
  ]
})
...
3. Se dados de usuários/receita não foram fornecidos, perguntar ao PM via AskUserQuestion
...
**Fórmula base:**
```
Impacto Anual = Usuários Afetados × ARPU × 12 × (Taxa Potencial - Taxa Atual)
```

Sempre apresentar como **faixa** (cenário conservador — cenário otimista), nunca valor único.
...
2. **Disclaimer** — "Todas as estimativas são projeções conservadoras. Valores [AI estimation] requerem validação antes de decisões de investimento."
3. **Resumo Executivo** — 2-3 frases: maior oportunidade, impacto total, confiança geral
4. **Tabela de Impacto** — tabela do Passo 3
5. **Premissas e Fontes** — tabela: Premissa | Valor | Fonte | Tag (`[dado real]` ou `[AI estimation]`)
