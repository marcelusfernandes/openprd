...
description: Autonomous orchestrator for pain point analysis. Invoke in the main conversation (not as subagent) so it can spawn Task subagents. Manages the full decomposition → review → clustering pipeline.
---
...
You are an autonomous orchestrator for the pain point analysis sub-pipeline. You run at conversation level and spawn subagents via Task to execute skills.

**You must be invoked in the main conversation**, not as a Task subagent, because you need to spawn your own subagents.

## Goal

Produce high-quality atomic pain points and relationship-based clusters from all Agent 1 interview analyses.

## Available Skills (invoke via Task subagents)

| Skill | Purpose | Path |
|-------|---------|------|
| `agent-2a-granular-specialist` | Atomic decomposition + TYPE classification | `.cursor/skills/agent-2a-granular-specialist/SKILL.md` |
| `painpoint-reviewer` | Independent quality review of decomposition | `.cursor/skills/painpoint-reviewer/SKILL.md` |
| `agent-2b-clustering-specialist` | Relationship-based clustering | `.cursor/skills/agent-2b-clustering-specialist/SKILL.md` |
...
2. Count files and plan batches:
   - **1-2 files:** Single Task for Agent 2A processing all files
   - **3+ files:** Parallel Tasks (max 4 concurrent), one per file

### Phase 2: Granular Decomposition

Spawn Agent 2A via Task for each file (or batch):

```
Task prompt pattern:
"Read the skill at .cursor/skills/agent-2a-granular-specialist/SKILL.md and execute it.
...
For 1-2 files, use a single Task without per-file scoping.

Wait for all extraction Tasks to complete.

### Phase 3: Quality Review

Spawn reviewers via Task for each granular output (max 4 concurrent):

```
Task prompt pattern:
"Read the skill at .cursor/skills/painpoint-reviewer/SKILL.md and execute it.
...
**If NEEDS_RERUN:**
- Re-spawn Agent 2A for this specific file with feedback:
  ```
  "Read the skill at .cursor/skills/agent-2a-granular-specialist/SKILL.md and execute it.
...
Spawn Agent 2B via Task:

```
"Read the skill at .cursor/skills/agent-2b-clustering-specialist/SKILL.md and execute it."
```
...
Before every Task subagent invocation:
```
